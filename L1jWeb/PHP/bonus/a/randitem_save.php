...
if($err_count==0){
	...
	$tranditempoint=$bonosranditempoint*$randitemc;
	$d=0;
	...
	$str="select count(*) from user_register where name='$id' and bonus_point<$tranditempoint";
...
if($err_count==0){
	...
	$str="update user_register set bonus_point=bonus_point-$tranditempoint where name='$id'";
	$db->query($str);
	...
	While($d<$randitemc){
		...
		if($randitemnum==0){
			$str="insert into `zwls_item_get_log` (`no` , `account` , `itemtype` , `objid` , `itemno` , `itemname` , `itemcount` , `point` , `logfrom` , `time` , `ip`) values (NULL,'$id','回饋轉蛋',NULL,NULL,'銘謝惠顧',NULL,'- $randitempoint','回饋轉蛋','$time','$ip')";
			$db->query($str);
		}else{
			...
			$str="insert into `zwls_item_get_log` (`no` , `account` , `itemtype` , `objid` , `itemno` , `itemname` , `itemcount` , `point` , `logfrom` , `time` , `ip`) values (NULL,'$id','回饋轉蛋','$randid','$randitemno','$rand +$randitemlv $randitemname','$randitemcount','- $randitempoint','回饋轉蛋','$time','$ip')";
			$db->query($str);
		}
...
$msg=$msg."購買完成<br>本次花費回饋點數 $tranditempoint 點<br>";
}
