...
import net.l1j.util.PropertyUtil;
...
public final class Config {
	...
	public static void load() {
		if (Server.serverMode == Server.MODE_GAMESERVER) {
			...
			try {
				// database.properties
				try {
					PropertyUtil databaseSettings = new PropertyUtil();
				...
				// server.properties
				try {
					PropertyUtil serverSettings = new PropertyUtil();
				...
				// general.properties
				try {
					PropertyUtil generalSettings = new PropertyUtil();
					...
					LOGGING_ITEM_DROP = Boolean.parseBoolean(generalSettings.getProperty("LoggingItemDrop", "false"));
				...
				try {
					PropertyUtil altSettings = new PropertyUtil();
				...
				try {
					PropertyUtil charSettings = new PropertyUtil();
				...
				// rates.properties
				try {
					PropertyUtil rateSettings = new PropertyUtil();
				...
				// thread.properties
				try {
					PropertyUtil threadSettings = new PropertyUtil();
					is = new FileInputStream(new File(THREAD_CONFIG_FILE));
					threadSettings.load(is);

					THREAD_P_TYPE_GENERAL = Integer.parseInt(threadSettings.getProperty("GeneralThreadPoolType", "2"), 10);
				...
				// version.properties
				try {
					PropertyUtil versionSettings = new PropertyUtil();
					is = new FileInputStream(new File(VERSION_CONFIG_FILE));
					versionSettings.load(is);

					SERVER_CORE_VERSION = versionSettings.getProperty("ServerCoreVersion", "Unknown");
					SERVER_BUILD_DATE = versionSettings.getProperty("ServerBuildDate", "Unknown");
				} catch (Exception e) {
					// Ignore PropertyUtil file if it doesnt exist
					SERVER_CORE_VERSION = "Unknown";
	...
	private static void validate() {
		if (!IntRange.includes(Config.ALT_ITEM_DELETION_RANGE, 0, 5)) {
			throw new IllegalStateException("ItemDeletionRange 的設定值可能超出(0 ~ 5)範圍。");
		}

		if (!IntRange.includes(Config.ALT_ITEM_DELETION_TIME, 1, 35791)) {
			throw new IllegalStateException("ItemDeletionTime 的設定值可能超出(1 ~ 35791)範圍。");
		}
	}
	...
	public static boolean setParameterValue(String pName, String pValue) {
		...
		} else if (pName.equalsIgnoreCase("ThreadPoolSizeEffects")) {
		...
		// version.properties
		else if (pName.equalsIgnoreCase("ServerCoreVersion")) {
			SERVER_CORE_VERSION = pValue;
		} else if (pName.equalsIgnoreCase("ServerBuildDate")) {
			SERVER_BUILD_DATE = pValue;
		} else {
			return false;
		}
		return true;
