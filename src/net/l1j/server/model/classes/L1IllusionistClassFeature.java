...
class L1IllusionistClassFeature extends L1ClassFeature {
	...
	private final static int points[] = { 11, 10, 12, 12, 8, 12, 10 }; // 力、敏、體、精、魅、智、自由點數
	@Override
	...
	public int calclvUpHp(int baseCon) {
		...
		int randomhp = baseCon * 5 / 6 + randomadd + 1; // 初期值分追加 4 <-> 9
			return randomhp;
	}
	...
	public int calclvUpMp(int BaseWis) {
		...
		int randommp = RandomArrayList.getInc(I_RandomMp[temp_BaseWis], I_BaseMp[temp_BaseWis]);
		return randommp * 5 / 3;
	}
