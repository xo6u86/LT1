...
public class L1PolyRace {
	...
	public void enterGame(L1PcInstance pc) {
		if (pc.getLevel() < 30) {
			pc.sendPackets(new S_ServerMessage(SystemMessageId.$1273, "30", "99"));
			return;
		}
	...
	public void addOrderList(L1PcInstance pc) {
		if (orderList.contains(pc)) {
			pc.sendPackets(new S_ServerMessage(SystemMessageId.$1254));
			return;
		}
		orderList.add(pc);
		pc.setInOrderList(true);
		pc.sendPackets(new S_ServerMessage(SystemMessageId.$1253, String.valueOf(orderList.size())));

		if (orderList.size() >= minPlayer) {
			for (L1PcInstance player : orderList) {
				player.sendPackets(new S_Message_YN(SystemMessageId.$1256, null));
			}
	...
	private void setGameStart() {
		...
		for (L1PcInstance pc : playerList) {
			speedUp(pc, 0, 0);
			randomPoly(pc, 0, 0);
			pc.sendPackets(new S_ServerMessage(SystemMessageId.$1257));
	...
	private void setGameEnd(int type) {
		...
		switch (type) {
			...
			case END_STATUS_NOPLAYER:
				for (L1PcInstance pc : playerList) {
					pc.sendPackets(new S_ServerMessage(SystemMessageId.$1264));
	...
	private void giftWinner() {
		...
		if (winner.getInventory().checkAddItem(item, 1) == L1Inventory.OK) {
			...
			winner.sendPackets(new S_ServerMessage(SystemMessageId.$403, item.getLogName()));
		}
	}
	...
	private void sendEndMessage() {
		...
		for (L1PcInstance pc : playerList) {
			if (winner != null) {
				pc.sendPackets(new S_ServerMessage(SystemMessageId.$1259));
	...
	public void removeSkillEffect(L1PcInstance pc) {
		...
		skill.handleCommands(pc, SkillId.SKILL_CANCEL_MAGIC, pc.getId(), pc.getX(), pc.getY(), null, 0, Base.SKILL_TYPE[1]);
	}
	...
	private class ReadyTimer extends TimerTask {
		...
		public void run() {
			for (L1PcInstance pc : playerList) {
				pc.sendPackets(new S_ServerMessage(SystemMessageId.$1258));
			}
			startCheckTimer();
			this.cancel();
		}
