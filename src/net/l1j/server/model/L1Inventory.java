...
public class L1Inventory extends L1Object {
	...
	public int checkAddItem(L1ItemInstance item, int count) {
		...
		if (getSize() > Config.MAX_NPC_ITEM || (getSize() == Config.MAX_NPC_ITEM && (!item.isStackable() || !checkItem(item.getItem().getItemId())))) { // 容量確認
			return SIZE_OVER;
		}
	...
	public synchronized L1ItemInstance storeItem(int id, int count) {
		...
		if (temp.isStackable()) {
			...
			if (findItemId(id) == null) { // 新しく生成する必要がある場合のみIDの發行とL1Worldへの登錄を行う
	...
	/**
	 * インベントリから指定されたアイテムIDのアイテムを削除する。L1ItemInstanceへの參照
	 * がある場合はremoveItemの方を使用するのがよい。 （こちらは矢とか魔石とか特定のアイテムを消費させるときに使う）
	 * 
	 * @param itemid - 削除するアイテムのitemid(objidではない)
	 * @param count - 削除する個數
	 * @return 實際に削除された場合はtrueを返す。
	 */
	...
	// 指定したアイテムから指定個數を削除（使ったりゴミ箱に捨てられたとき）戾り值：實際に削除した數
	...
	// 特定のアイテムを指定された個數以上所持しているか確認（矢とか魔石の確認）
	...
	/**
	 * このインベントリ內にある、指定されたIDのアイテムの數を數える。
	 * 
	 * @return
	 */
